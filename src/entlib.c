...
// This is too large in almost all cases, but in the instance that the value is close to zero, it can go this badly.
// In practice, it's unlikely that any of these are going much past 53 rounds (as that's the size of the
// mantissa component of the double)
#define ITERMAX 1076
...
/*fval is assumed to be a monotonic function within the interval provided*/
/*It's monotonic down if "decreasing" is true, and monotonic up otherwise*/
/*The invariant that is verified in each step is that the target lies between the bounds*/
/*The input values bounds (ldomain, hdomain) are not assumed to be in the domain*/
/*The function is assumed to be well behaved (returning finite values) within its domain*/
/*In the event that we don't come sufficiently close, return the largest (reasonable) upper bound*/
/*which translates to the lowest reasonable assessed entropy*/
/*-1 returned if there is a different error*/
static double monotonicBinarySearch(double (*fval)(double, const size_t *), double ldomain, double hdomain, double target, const size_t *params, bool decreasing) {
  double center;
  double lastCenter;
  double centerVal;
  uint32_t j;
  ...
  if (configVerbose > 3) {
    fprintf(stderr, "Seeking value %.17g with a relative factor of %.17g\n", target, RELEPSILON);
  }

  assert(ldomain < hdomain);
  ...
  assert(INOPENINTERVAL(center, ldomain, hdomain));

  centerVal = fval(center, params);

  // Invariant: centerVal is finite
  if (!isfinite(centerVal)) {
    // center was within the domain, so this should never happen.
    fprintf(stderr, "CenterVal (%.17g) is not finite for initial center.\n", centerVal);
    return (-1.0);
  }

  // We don't need the initial centerVal invariant, as our initial bounds are infinite.
  // We don't need the initial bounds, as they are set to the domain bounds

  for (j = 0; j < ITERMAX; j++) {
    // Have we reached "equality"?
    if (relEpsilonEqual(centerVal, target, ABSEPSILON, RELEPSILON, ULPEPSILON)) {
      // Yes, they are equal
      if (configVerbose > 3) {
        fprintf(stderr, "Close enough after %u rounds; x: %.17g, target: %.17g, value: %.17g\n", j + 1, center, target, centerVal);
      }
      return (center);
    } else {
      // No, they are not equal
      if (configVerbose > 3) {
        fprintf(stderr, "Round %u; x: %.17g, target: %.17g, value: %.17g, rel-delta: %.17g, abs-delta: %.17g\n", j + 1, center, target, centerVal, RELFACTOR(centerVal, target), fabs(centerVal - target));
      }
    }

    // Now update based on the found centerVal
    if (target < centerVal) {
      if (decreasing) {
        // right hand side
        lbound = center;
    ...
    if (lbound >= hbound) {
      if (configVerbose > 3) {
        fprintf(stderr, "Upper and lower bounds have converged after %u rounds and target was not found. Returning the largest bound.\n", j + 1);
      }
      return (fmin(fmax(lbound, hbound), hdomain));
    }
    ...
    if (!(INCLOSEDINTERVAL(lbound, ldomain, hdomain) && INCLOSEDINTERVAL(hbound, ldomain, hdomain))) {
      fprintf(stderr, "The current search interval is not a subset of the domain after %u.\n", j + 1);
      return (-1.0);
    }

    // invariant. If this isn't true, then seeking the value within this interval doesn't make sense.
    if (!INCLOSEDINTERVAL(target, lvalue, hvalue)) {
      fprintf(stderr, "Target is not within the search interval after %u rounds.", j + 1);
      return (-1.0);
    }

    // Update the center
    // Note the values are in the interval [0,1], so overflow isn't an issue, but underflow is.
    lastCenter = center;
    center = (lbound + hbound) / 2.0;

    // invariant. If this isn't true, then further calculation isn't really meaningful.
    if (!INOPENINTERVAL(center, lbound, hbound)) {
      if (configVerbose > 3) {
        fprintf(stderr, "The next center is outside of the search interval we're exploring after %u rounds. Returning upper bound.\n", j + 1);
      }
      return (hbound);
    }
...
#pragma GCC diagnostic ignored "-Wfloat-equal"
    ...
    if (lastCenter == center) {
      if (configVerbose > 1) {
        fprintf(stderr, "Detected cycle after %u rounds. Returning upper bound.\n", j + 1);
      }
      return (hbound);
    }
#pragma GCC diagnostic pop
    ...
    if (!isfinite(centerVal)) {
      // The center is within the domain, so this should never occur
      fprintf(stderr, "CenterVal (%.17g) is not finite after %u rounds.\n", centerVal, j + 1);
      return (-1.0);
    }

    // invariant: If this isn't true, then this isn't loosely monotonic
    if (!INCLOSEDINTERVAL(centerVal, lvalue, hvalue)) {
      if (configVerbose > 3) {
        fprintf(stderr, "CenterVal (%.17g) is not within the search value interval after %u rounds. Returning upper bound.\n", centerVal, j + 1);
      }
      return (hbound);
    }
  }
  ...
  if (relEpsilonEqual(centerVal, target, ABSEPSILON, RELEPSILON, ULPEPSILON)) {
    if (configVerbose > 3) {
      fprintf(stderr, "Fell through, but close enough\n");
    }
    return (center);
  } else {
    if (configVerbose > 3) {
