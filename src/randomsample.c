...
noreturn static void useageExit(void) {
  ...
  fprintf(stderr, "randomsample inputfile outputSamples\n");
  fprintf(stderr, "Produce a random sample of the input file of size outputSamples.\n");
  fprintf(stderr, "inputfile is assumed to be a stream of uint64_ts in decimal format, one per line.\n");
...
int main(int argc, char *argv[]) {
  ...
  datalen = readasciiuint64s(infp, &data);
  if (datalen < 1) {
    useageExit();
  }
